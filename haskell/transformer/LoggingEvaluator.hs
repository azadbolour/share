...
{-
  Evaluator that has the environment factored out and profiles and logs evaluation operations.
-}

-- See README for signatures and meanings of the transformer types used.
...
type EffectfulVal val = ReaderT Env (ExceptT String (WriterT [String] (StateT OperationCount Identity))) val

runEffectfulVal :: Env -> OperationCount -> EffectfulVal val -> ((Either String val, [String]), OperationCount)
runEffectfulVal env = \initialCount effectfulVal -> runIdentity(runStateT(runWriterT(runExceptT (runReaderT effectfulVal env))) initialCount)
