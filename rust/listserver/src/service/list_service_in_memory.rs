...
impl ListService for ListServiceInMemory {
    ...
    fn add_element(&self, id: &ID, element: Element, index: usize) -> Result<(), ListError> {
        ...
        match lock.get(&id.clone()) {
            Some(safe_list) => {
                self.insert_into_safe_list(safe_list, id, element, index).map(|list| {
    ...
    fn get_element(&self, id: &ID, index: usize) -> Result<Option<Element>, ListError> {
        self.get(id).and_then(|bare_list|
            Ok(bare_list.get(index).map(|el| el.clone()))
        )
    }
...
impl ListServiceInMemory {
    ...
    fn do_insert(&self, id: &String, lock: &mut MutexGuard<UnsafeListStore>) -> Result<(), ListError> {
        ...
        match lock.insert(id.clone(), safe_list) {
            None => Ok(()),
            Some(_) => Err(ListError::ListInternalError("unable to insert list into list store".to_string()))
        }
    }
    ...
    fn insert_into_safe_list(&self, save_list: &Mutex<Vec<String>>, id: &String, element: String, index: usize) -> Result<SafeList, ListError> {
