...
open import UtilProperties using (≤⇒<suc; <-step; <-suc⇒≤; <-suc;
...
-- hint - for the inductice case using suc x, x is bounded by suc x < suc k
--        case split on the bound to get x<k and base the proof entirely on x and x<k
--        use the succesor equality lemma applicable to x<k?
